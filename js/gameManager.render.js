...
export function drawImpl(mouseEvent) {
    // 1. 좌표계 초기화 및 배경 지우기 (고해상도 전체 지움)
    this.ctx.setTransform(1, 0, 0, 1, 0, 0);
    this.ctx.fillStyle = '#1f2937';
    this.ctx.fillRect(0, 0, this.canvas.width, this.canvas.height);

    // 2. 배율 적용 (이후 모든 그림은 3배 커짐)
    const resScale = this.resolutionScale || 3;
    this.ctx.scale(resScale, resScale);
    this.ctx.imageSmoothingEnabled = false;

    this.ctx.save();

    // 3. 카메라 중앙 정렬 (논리적 크기 기준)
    const logicalW = this.logicalWidth || (this.canvas.width / resScale);
    const logicalH = this.logicalHeight || (this.canvas.height / resScale);

    const cam = this.actionCam;
    
    // 화면의 논리적 중앙으로 이동 -> 카메라 줌 적용 -> 카메라 위치(World)로 역이동
    ...
    this.drawPoisonPuddles(this.ctx);
    ...
    if (this.isNametagSwapMode && this.draggedUnitForSwap && mouseEvent) {
        ...
        if (unit.name) {
            ...
            const logicalX = (mouseEvent.clientX - rect.left) * (logicalW / rect.width);
            const logicalY = (mouseEvent.clientY - rect.top) * (logicalH / rect.height);

            this.ctx.font = 'bold 10px Arial';
            this.ctx.textAlign = 'center';
            this.ctx.fillStyle = unit.nameColor || '#000000';
            this.ctx.fillText(unit.name, logicalX, logicalY - 20);
            this.ctx.restore();
        }
    }
    ...
    if (this.actionCam.vignetteEnabled && (this.isActionCam || this.isFollowCamEnabled) && cam.current.scale > 1.05) {
        ...
        const outerRadius = Math.hypot(logicalW, logicalH) / 2;
        const gradient = this.ctx.createRadialGradient(logicalW / 2, logicalH / 2, logicalH / 2.5, logicalW / 2, logicalH / 2, outerRadius);
