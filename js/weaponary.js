        ...
        if (this.type === 'sword') {
            ...
            if (unit.attackCount >= 3) {
                ...
                for (let i = 0; i < 15; i++) {
                    ...
                    const speed = 1 + gameManager.visualPrng.next() * 2;
                    gameManager.addParticle({
                        x: unit.pixelX, y: unit.pixelY,
                        vx: Math.cos(angle) * speed, vy: Math.sin(angle) * speed,
                        life: 0.7,
                        color: teamColor,
                        size: gameManager.visualPrng.next() * 2 + 1,
                        gravity: 0.05
                    });
                }
            }
...
export function createPhysicalHitEffect(gameManager, target) {
    ...
    for (let i = 0; i < particleCount; i++) { // This is a visual effect, should not affect simulation
        const angle = gameManager.visualPrng.next() * Math.PI * 2;
        const speed = 2 + gameManager.visualPrng.next() * 3;
        gameManager.addParticle({
            x: target.pixelX,
            y: target.pixelY,
            ...
            life: 0.7,
            color: '#ef4444',
            size: gameManager.visualPrng.next() * 2.5 + 1.5,
            gravity: 0.1
        });
    }
}
...
export function createFireballHitEffect(gameManager, x, y) {
    ...
    for (let i = 0; i < particleCount; i++) { // This is a visual effect, should not affect simulation
        const angle = gameManager.visualPrng.next() * Math.PI * 2;
        const speed = 1 + gameManager.visualPrng.next() * 4;
        gameManager.addParticle({
            x: x,
            y: y,
            vx: Math.cos(angle) * speed,
            vy: Math.sin(angle) * speed,
            life: 0.6, // 지속 시간 감소
            color: ['#ffcc00', '#ff9900', '#ff6600', '#ef4444'][Math.floor(gameManager.visualPrng.next() * 4)],
            size: gameManager.visualPrng.next() * 3 + 2,
...
export class Projectile {
    ...
    handleSpecialArrowTrail() {
        if (this.gameManager.visualPrng.next() > 0.3) { // 파티클 생성 빈도 조절
            this.gameManager.addParticle({
                x: this.pixelX,
                y: this.pixelY,
                vx: (this.gameManager.visualPrng.next() - 0.5) * 1.5,
                vy: (this.gameManager.visualPrng.next() - 0.5) * 1.5,
                life: 0.4,
                color: this.gameManager.visualPrng.next() > 0.5 ? '#facc15' : '#fb923c', // 노랑/주황
                size: this.gameManager.visualPrng.next() * 2 + 1,
                gravity: 0
            });
        }
    }
    ...
    handleSwordWaveTrail() {
        if (this.gameManager.visualPrng.next() > 0.4) {
            this.gameManager.addParticle({
                x: this.pixelX, y: this.pixelY,
                vx: (this.gameManager.visualPrng.next() - 0.5) * 1.5,
                vy: (this.gameManager.visualPrng.next() - 0.5) * 1.5,
                life: 0.5,
                color: this.gameManager.visualPrng.next() > 0.5 ? '#ef4444' : '#fca5a5',
                size: this.gameManager.visualPrng.next() * 2 + 1,
                gravity: 0.02
            });
        }
    ...
    handleBouncingSwordTrail() {
        if (this.gameManager.visualPrng.next() > 0.3) {
            this.gameManager.addParticle({
                x: this.pixelX, y: this.pixelY,
                vx: (this.gameManager.visualPrng.next() - 0.5) * 1.0,
                vy: (this.gameManager.visualPrng.next() - 0.5) * 1.0,
                life: 0.4,
                color: '#9ca3af',
                size: this.gameManager.visualPrng.next() * 1.5 + 1,
                gravity: 0
            });
        }
    }
    ...
    handleBoomerangTrail() {
        if (this.gameManager.visualPrng.next() > 0.4) {
            ...
            this.gameManager.addParticle({
                x: this.pixelX,
                y: this.pixelY,
                vx: (this.gameManager.visualPrng.next() - 0.5) * 1.2,
                vy: (this.gameManager.visualPrng.next() - 0.5) * 1.2,
                life: 0.5,
                color: teamColor,
                size: this.gameManager.visualPrng.next() * 2 + 1,
                gravity: 0
    ...
    // [NEW] 장풍 파티클 효과
    handleHadokenTrail() {
        if (this.gameManager.visualPrng.next() > 0.3 && this.owner) {
            ...
            this.gameManager.addParticle({
                x: this.pixelX, y: this.pixelY,
                vx: (this.gameManager.visualPrng.next() - 0.5) * 2,
                vy: (this.gameManager.visualPrng.next() - 0.5) * 2,
                life: 0.5,
                color: this.gameManager.visualPrng.next() > 0.5 ? particleColor1 : particleColor2,
                size: this.gameManager.visualPrng.next() * 2.5 + 1,
                gravity: 0
            });
        }
    }
    ...
    handleLightningTrail() {
        if (this.gameManager.visualPrng.next() > 0.2) {
            this.gameManager.addParticle({
                x: this.pixelX, y: this.pixelY,
                vx: (this.gameManager.visualPrng.next() - 0.5) * 1.5,
                vy: (this.gameManager.visualPrng.next() - 0.5) * 1.5,
        ...
        if (this.type === 'shuriken' || this.type === 'returning_shuriken') {
            ...
            if (this.gameManager.visualPrng.next() > 0.3) {
                this.gameManager.addParticle({
                    ...
                    vy: (this.gameManager.visualPrng.next() - 0.5) * 1.5,
                    life: 0.6,
                    color: ['#86efac', '#4ade80', '#22c55e'][Math.floor(this.gameManager.visualPrng.next() * 3)],
                    size: this.gameManager.visualPrng.next() * 2 + 1.5,
                    gravity: -0.02
                });
            }
...
export class Effect {
    constructor(gameManager, x, y, type, target, options = {}) {
        ...
        this.angle = this.gameManager.visualPrng.next() * Math.PI * 2;
        ...
        if (this.type === 'question_mark_effect') {
            ...
            for (let i = 0; i < 20; i++) {
                this.particles.push({
                    x: this.x, y: this.y,
                    angle: this.gameManager.visualPrng.next() * Math.PI * 2,
                    speed: this.gameManager.visualPrng.next() * 2 + 1,
                    radius: this.gameManager.visualPrng.next() * 3 + 1,
                    lifespan: 40,
                });
            }
...
export class AreaEffect {
    constructor(gameManager, x, y, type, options = {}) {
        ...
        if (this.type === 'fire_pillar') {
            for (let i = 0; i < 50; i++) {
                this.particles.push({
                    x: (this.gameManager.visualPrng.next() - 0.5) * this.maxRadius * 1.5,
                    y: (this.gameManager.visualPrng.next() - 0.5) * this.maxRadius * 0.5,
                    size: this.gameManager.visualPrng.next() * 4 + 2,
                    speed: this.gameManager.visualPrng.next() * 1.5 + 1,
                    lifespan: this.gameManager.visualPrng.next() * 20 + 10,
                    color: ['#ffcc00', '#ff9900', '#ff6600', '#ef4444'][Math.floor(this.gameManager.visualPrng.next() * 4)]
                });
            }
        }
    }
...
export class Projectile {
    ...
    draw(ctx) {
        if (this.type === 'shuriken' || this.type === 'returning_shuriken') {
            ...
            const grad = ctx.createRadialGradient(this.pixelX, this.pixelY, radius * 0.2, this.pixelX, this.pixelY, radius);
            grad.addColorStop(0, 'rgba(255, 255, 255, 1.0)');
            grad.addColorStop(0.5, `rgba(, 0.9)`);
            ...
            for (let i = 0; i < this.trail.length; i++) {
                ...
                ctx.fillStyle = `rgba(192, 132, 252, )`;
                ctx.beginPath();
                ctx.arc(trailX, trailY, (GRID_SIZE / 4) * (i / this.trail.length), 0, Math.PI * 2);
                ctx.fill();
            }
            ...
            this.trail.forEach((pos, index) => {
                const opacity = (index / this.trail.length) * 0.4;
                ctx.fillStyle = teamColor.replace(')', `, )`).replace('#', 'rgba(').replace(')', '');
                ctx.beginPath();
                const size = (GRID_SIZE / 3) * (index / this.trail.length);
                ctx.arc(pos.x, pos.y, size, 0, Math.PI * 2);
                ctx.fill();
            ...
            for (let i = 0; i < this.trail.length; i++) {
                const pos = this.trail[i];
                const alpha = (i / this.trail.length) * 0.5;
                ctx.fillStyle = `rgba(165, 243, 252, )`;
                ctx.beginPath();
                ctx.arc(pos.x, pos.y, (GRID_SIZE / 1.67) * (i / this.trail.length), 0, Math.PI * 2);
                ctx.fill();
            }
...
export class Effect {
    ...
    draw(ctx) {
        ...
        if (this.type === 'slash' || this.type === 'dual_sword_slash') {
            ...
            ctx.strokeStyle = `rgba(254, 240, 138, )`;
            ctx.lineWidth = 2;
            ctx.beginPath();
            ctx.moveTo(this.x, this.y);
            ctx.lineTo(this.target.pixelX, this.target.pixelY);
            ...
            ctx.save();
            ctx.translate(this.x, this.y);
            ctx.strokeStyle = this.options.color || `rgba(220, 38, 38, )`;
            ctx.lineWidth = this.options.lineWidth || 4;
            ctx.beginPath();
            ctx.arc(0, 0, radius, 0, Math.PI * 2);
            ctx.stroke();
